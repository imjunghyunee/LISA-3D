#!/bin/bash
# ──────────────────────────────────────────────────────────────────────────────
# LISA-3D Path B+ — LoRA + text_hidden_fcs + mask_decoder + lm_head + embed_tokens.
#
# In addition to Path B, this also unfreezes the LLaMA output layers
# (lm_head and embed_tokens, including the [SEG] token row).  Maximum
# trainable capacity for domain adaptation; correspondingly higher VRAM.
# Effective batch size is preserved by raising grad_accum.
#
...
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 ./run_train_Bplus.sh \
...
LORA_DROPOUT="0.05"
UNFREEZE_MODE="B+"
HEAD_LR_SCALE="0.2"     # heads run at lr * head_lr_scale
...
# ── Print configuration ───────────────────────────────────────────────────────
echo "LISA-3D Path B+ — LoRA + heads + lm_head + embed_tokens"
echo "─────────────────────────────────────────────────────────────────────"
...
echo "Unfreeze mode       : $UNFREEZE_MODE (LoRA + heads + lm_head + embed_tokens)"
echo "Epochs              : $EPOCHS"
