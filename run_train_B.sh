#!/bin/bash
# ──────────────────────────────────────────────────────────────────────────────
# LISA-3D Path B — LoRA + text_hidden_fcs + SAM mask_decoder training.
#
# Trains the LoRA adapters PLUS the SEG→SAM-prompt projection MLP
# (text_hidden_fcs) and the SAM ViT-H mask_decoder.  This matches the
# trainable subset of LISA's original train_ds.py (minus the LLM head)
# while keeping LISA-3D's multi-view geometric consistency loss.
#
...
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 ./run_train_B.sh \
...
LORA_DROPOUT="0.05"
UNFREEZE_MODE="B"
HEAD_LR_SCALE="0.3"     # heads run at lr * head_lr_scale
...
# ── Print configuration ───────────────────────────────────────────────────────
echo "LISA-3D Path B — LoRA + text_hidden_fcs + SAM mask_decoder"
echo "─────────────────────────────────────────────────────────────────────"
...
echo "Unfreeze mode       : $UNFREEZE_MODE (LoRA + text_hidden_fcs + mask_decoder)"
echo "Epochs              : $EPOCHS"
